...
class HandSequence:
    ...
    def grasp_cylinder(self):
        ...
        for i in range(7):
            if i == 0:
                ...
                targets.append(self.maxLimits[i] if self.CLOSE_DIR[i] == 1 else self.minLimits[i])

        while True:
            if self.get_pressure() > self.PRESSURE_THRESHOLD:
                break
    ...
    def release(self):
        self.mode = "normal"
        self.apply_positions({f"mano_izq_{i}": 0.0 for i in range(7)}, duration=1.0)


# =====================================
# ARM CONTROL (CONTINUO REAL)
# =====================================
class ArmSequence:
...
def main():
    ...
    print("[SYSTEM] Ejecución continua activada")
    ...
    for paso in pasos:
        ...
        # ========= MANO =========
        pos_left = {k:v for k,v in pos.items() if "mano_izq" in k}

        if pos_left and hand.mode != "grasp":
            hand.apply_positions(pos_left, duration=dur)
